...
function buildPrintUrl(baseUrl, nis, filters) {
    ...
    if (!query) return `${baseUrl}/laporan/cetak-santri/${nis}/pdf`;
...
export default function CetakSantriPage(props) {
    ...
    const submit = (e) => {
        e.preventDefault();
        router.get(`${baseUrl}/laporan/cetak-santri`, cleanParams(localFilters), {
            preserveState: true,
            replace: true,
        });
    };
            ...
            <div className="space-y-4">
                ...
                {!canLoadData && (
                    <div className="rounded-md border border-yellow-300 bg-yellow-50 p-4 text-sm text-yellow-700">
                        Pilih pondok terlebih dahulu untuk mencetak data santri.
                    </div>
                )}
