...
const ustadzEdit = ({ ustadz, user }) => {
    const [loading, setLoading] = useState(false);
    console.log(ustadz);

    // Inisialisasi data form, ambil dari props ustadz dan user
    const { data, setData, errors, post, processing } = useForm({
        _method: 'PUT',
        nip: ustadz.nip || '',
        nama: ustadz.nama || '',
        gelar_awal: ustadz.gelar_awal || '',
        gelar_akhir: ustadz.gelar_akhir || '',
        tempat_lahir: ustadz.tempat_lahir || '',
        tanggal_lahir: ustadz.tanggal_lahir || '',
        jenis_kelamin: ustadz.jenis_kelamin || '',
        status_menikah: ustadz.status_menikah == 1 ? true : false,
        alamat: ustadz.alamat || '',
        no_identitas: ustadz.no_identitas || '',
        no_telpon: ustadz.no_telpon || '',
        no_handphone: ustadz.no_handphone || '',
        // Data akun
        username: user?.username || '',
        email: user?.email || '',
        password: '',
        tanggal_kerja: ustadz.tanggal_kerja || '',
        non_aktif: ustadz.non_aktif == 1 ? true : false,
        keterangan: ustadz.keterangan || '',
        id: ustadz.id,
    });
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        setLoading(true);
        post(route('admin-cabang.ustadz.update', ustadz.id), {
    ...
    return (
        <Layout title={`Edit ustadz - ${data.nama}`}>
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <h2 className="text-lg font-semibold text-gray-700">Form Edit ustadz</h2>
                    <Link
                        href={`/admin-cabang/ustadz/${data.id}`}
