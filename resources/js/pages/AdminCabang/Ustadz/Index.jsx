...
const ustadzIndex = () => {
    const { ustadzs, flash } = usePage().props;
    ...
    // Data paginated
    const ustadzsData = ustadzs.data;

    // Filter hanya pada data halaman saat ini
    const filtered = useMemo(() => {
        if (!ustadzsData) return [];
        const q = keyword.toLowerCase();
        return ustadzsData.filter((d) => 
    ...
    const handleDelete = (id) => {
        ...
        router.delete(route('admin-cabang.ustadz.destroy', id), {
            preserveScroll: true
        });
    };
        ...
        },
        {
            ...
            render: (row) => (
                <div className="space-x-2 whitespace-nowrap">
                    <Link href={`/admin-cabang/ustadz/${row.id}`} className="text-blue-600 hover:text-blue-900">
                        Detail
                    </Link>
                    <Link href={`/admin-cabang/ustadz/${row.id}/edit`} className="text-yellow-600 hover:text-yellow-900">
                        Edit
                    </Link>
            ...
            <div className="space-y-4">
                ...
                <div className="rounded-lg bg-white p-4 shadow-md">
                    <div className="mb-3 flex items-center justify-between">
                        <h2 className="text-lg font-semibold text-gray-700">Daftar ustadz</h2>
                        <Link
                            href="/admin-cabang/ustadz/create"
                    ...
                    {filtered && filtered.length > 0 ? (
                        ...
                        <div className="text-center py-10">
                            <p className="text-gray-500">
                                {keyword ? 'Tidak ada ustadz yang sesuai dengan pencarian.' : 'Belum ada data ustadz. Silakan tambahkan ustadz baru.'}
                            </p>
                        </div>
                    )}
                    ...
                    <div className="mt-4 flex justify-between items-center">
                        <div className="text-sm text-gray-500">
                            Menampilkan {ustadzs.from} sampai {ustadzs.to} dari {ustadzs.total} ustadz
                        </div>
                        <div className="flex space-x-2">
                            {ustadzs.links.map((link, idx) => {
