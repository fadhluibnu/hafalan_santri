...
const ustadzShow = ({ ustadz }) => {
    ...
    const handleDelete = () => {
        ...
        setDeleting(true);
        router.delete(`/admin-cabang/ustadz/${ustadz.id}`, {
    ...
    return (
        <Layout title={`Detail ustadz - ${ustadz.nama}`}>
            <div className="space-y-4">
                <div className="rounded-lg bg-white p-6 shadow-md">
                    ...
                    <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                        <div>
                            ...
                            <dd className="mt-1 text-sm text-gray-900">
                                {(ustadz.gelar_awal ? ustadz.gelar_awal + ' ' : '') + 
                                 ustadz.nama + 
                                 (ustadz.gelar_akhir ? ', ' + ustadz.gelar_akhir : '')}
                            </dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">NIP</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.nip || '-'}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">Tempat, Tanggal Lahir</dt>
                            <dd className="mt-1 text-sm text-gray-900">
                                {ustadz.tempat_lahir || '-'}, {formatDate(ustadz.tanggal_lahir)}
                            </dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">Jenis Kelamin</dt>
                            <dd className="mt-1 text-sm text-gray-900">{labelJK(ustadz.jenis_kelamin)}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">Status Menikah</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.status_menikah == true ? 'Sudah Menikah' : 'Belum Menikah'}</dd>
                        </div>
                        
                        <div className="md:col-span-2">
                            <dt className="text-sm font-medium text-gray-500">Alamat</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.alamat || '-'}</dd>
                        </div>
                    </dl>
                    ...
                    <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                        <div>
                            ...
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.no_identitas || '-'}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">No. Telepon</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.no_telpon || '-'}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">No. Handphone</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.no_handphone || '-'}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">Email</dt>
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.email || '-'}</dd>
                        </div>
                    </dl>
                    ...
                    <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3">
                        <div>
                            ...
                            <dd className="mt-1 text-sm text-gray-900">{formatDate(ustadz.tanggal_kerja)}</dd>
                        </div>
                        
                        <div>
                            <dt className="text-sm font-medium text-gray-500">Status</dt>
                            <dd className="mt-1 text-sm text-gray-900">
                                <span className={`px-2 py-1 rounded-full text-xs ${ustadz.non_aktif ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>
                                    {ustadz.non_aktif == 0 ? 'Aktif' : 'Non-Aktif'}
                                </span>
                            </dd>
                        </div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <dd className="mt-1 text-sm text-gray-900">{ustadz.keterangan || '-'}</dd>
                        </div>
                    </dl>
                </div>
                ...
                <div className="flex items-center justify-end space-x-3">
                    ...
                    <Link
                        href={`/admin-cabang/ustadz/${ustadz.id}/edit`}
